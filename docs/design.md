...
6. When `--mode artifact` or `--mode install` is set and metadata policy does not block, `internal/artifact` downloads one selected artifact into a temporary file, enforces limits, and verifies its digest.
...
10. In `--mode install`, `internal/installer` invokes the real package manager with the exact selected package spec only if final policy does not block.
...
internal/installer/      Real package-manager invocation for install mode
...
Common fields include ecosystem, registry, package name, selected version, publish timestamps, description, license, author, maintainers, project URLs, version count, policy summary, decision, findings, optional install summary, and optional debug trace entries.

Adapters may attach an internal-only artifact candidate containing its registry URL and verification data. Public reports expose optional artifact-download, artifact-inspection, artifact-delta, and install summaries; temporary paths and download URLs are never rendered. Archive file paths used for delta comparison are internal-only and are not emitted in JSON. Install summaries do not include raw package-manager stdout or stderr.
...
- Artifact-delta checks enable selection and download of the immediate previous comparable artifact in artifact and install modes.
...
`--mode install` is the only mode that invokes a package manager. It always runs metadata checks and selected root-artifact verification/inspection before install. If metadata or artifact policy produces `BLOCK`, SourceGate skips the package-manager command and exits `30`.

When policy allows, SourceGate invokes the package manager without a shell:

- npm installs use `npm install <name>@<selected-version>`.
- PyPI installs use `pip install <name>==<selected-version>`.

Successful installs preserve SourceGate policy exit codes: `0` clean, `10` inform, and `20` alert. Package-manager failure or timeout is reported as operational exit code `2` with a bounded install summary. Install mode currently gates only the requested root package; transitive dependencies are still resolved by the package manager and are not recursively gated.
...
- Recursively resolve and gate all transitive dependencies before install.
- Suppress package-manager lifecycle scripts or Python build hooks after policy allows install.
- Perform runtime malware analysis.
- Verify remote source repositories or maintainer identity outside registry-provided metadata.

`--mode install` intentionally preserves normal package-manager behavior after SourceGate allows the requested root package. Safer package-manager flags and transitive dependency gating are tracked as follow-up work.
